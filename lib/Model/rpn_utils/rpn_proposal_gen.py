...
class RPNProposalGenerator(nn.Module):
    ...
    def subsample(self, confs):
        ...
        if num_pos > min_pos_samples:
            ...
            replace = num_pos_idx.shape[0] < min_pos_samples
            rand_remove = torch.multinomial(num_pos_idx, num_samples=num_pos_idx.shape[0] - min_pos_samples, replacement=replace)
            confs.view(-1)[rand_remove] = -1
        
        if num_neg > min_neg_samples:
            num_neg_idx = torch.where(confs.view(-1) == 0)[0].float()
            replace = num_neg_idx.shape[0] < min_neg_samples
            rand_remove = torch.multinomial(num_neg_idx, num_samples=num_neg_idx.shape[0] - min_neg_samples, replacement=replace)
            confs.view(-1)[rand_remove] = -1
